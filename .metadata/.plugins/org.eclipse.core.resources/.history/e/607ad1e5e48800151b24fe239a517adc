...
public class FileWorker 
{
			...
			while(drinkListLine != null && end == false)
			{
				if(firstLine)//reads first line aka the first drink on file
				{
					...
					for(int x = 0; x<=pumpCount-1; x++)//hold constant the drink list line for each line of the mixers
					{
						...
						if(Integer.parseInt(tempLine[tempLine.length-1])!=0)//if the amount for the mix is not zero check for that mixer.
						{
	...
	public int[] getDrinkMakeUp(String drinkToMake)
	{
		int[] drinkMixerAmount = new int[pumpCount];
		...
		{ 
			...
			drinkMixerAmount[x] = Integer.parseInt(tempString2[tempString2.length-1]);//splits each index of array tempString, parses the string at the last index (the part containing the amount) to an integer.
